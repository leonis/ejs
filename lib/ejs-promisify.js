  ...
  constructor() {
    ...
    */
    ...
    this.render = (template, locals, opts) => {
      ...
      let source = templ.source;
      // yield化
      const pattern = '(^((?:(?!__append).)*)$)';
      source = source.replace(new RegExp(pattern, 'gm'), ($0) => {
        $0 = $0.replace(/request\(/g, 'yield request(');
        $0 = $0.replace(/redirect\(/g, 'yield redirect(');
        $0 = $0.replace(/requests\(/g, 'yield requests(');
        $0 = $0.replace(/includeTemplate\(/g, 'yield includeTemplate(');
        return $0;
      });
