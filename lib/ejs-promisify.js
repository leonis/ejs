  ...
  constructor() {
    ...
    */
    this.request = (method, uri, options) => {
      ...
      params.headers['content-length'] = (params.form) ? params.form.length : params.body.length;
    ...
    */
    this.includeTemplate = (name) => {
      return co(function * () {
        ...
        return yield self.render(template, self.locals, self.opts);
      });
    };
    ...
    this.render = (template, locals, opts) => {
      ...
      const source = templ.source.replace(/>\\n</g, '><').replace(/>\\n"\)/g, '>")');
      ...
      Object.keys(GLOBAL).forEach((key) => {
        self.locals[key] = undefined;
      });

      // おまけ
      self.locals.eval = undefined;
      self.locals.Function = undefined;
      ...
      return fn.call(self.locals, self.redirect, self.request, self.requests, self.includeTemplate, co, utils.escapeXML, rethrow);
    };
  }
}();
